...
import com.obs.services.model.{ListObjectsRequest, ObsObject}
...
final case class ObsStorageFileSystemProvider(config: ObsStorageConfiguration) extends FileSystemProvider {
  ...
  class PathIterator(obsClient: ObsClient, prefix: ObsStoragePath, filter: DirectoryStream.Filter[_ >: Path]) extends AbstractIterator[Path] {
    ...
    private[this] def listNext(marker: String): Iterator[String] = {
      ...
      objectListing.getObjects.asScala.filterNot(_.equals(prefix.key)).foreach(obj => {
        result append obj.getObjectKey.stripPrefix(prefix.key)
      })
      objectListing.getCommonPrefixes.asScala.filterNot(_.equals(prefix.key)).foreach(obj => {
        result append obj.stripPrefix(prefix.key)
      })
  ...
  override def newFileSystem(uri: URI, env: util.Map[String, _]): ObsStorageFileSystem = {
    ...
    ObsStorageFileSystem(this, bucket, ObsStorageConfiguration.parseMap(env.asScala.mapValues(_.asInstanceOf[Any]).toMap))
  }
