...
import cromwell.filesystems.obs.nio._
...
object ObsPathBuilder {
  def fromConfiguration(endpoint: String,
    ...
    val obsStorageConfiguration = new ObsStorageConfiguration(endpoint, accessKey, secretKey)
    ObsPathBuilder(obsStorageConfiguration)
  }

  //obs-cromwell.obs.cn-north-1.myhwclouds.com
  ...
  case object PossiblyValidRelativeObsPath extends ObsPathValidation
    ...
    Try {
      ...
      if (uri.getScheme == null) PossiblyValidRelativeObsPath
        ...
        if (uri.getHost == null) {
          ...
          } getOrElse InvalidFullObsPath(string)
...
final case class ObsPathBuilder(obsStorageConfiguration: ObsStorageConfiguration) extends PathBuilder {
  def build(string: String): Try[ObsPath] = {
    validateObsPath(string) match {
      ...
      case PossiblyValidRelativeObsPath => Failure(new IllegalArgumentException(s"$string does not have a obs scheme"))
