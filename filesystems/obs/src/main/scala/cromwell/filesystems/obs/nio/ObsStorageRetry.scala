...
object ObsStorageRetry {
  def fromTry[A](f: () => Try[A],
    ...
    f() match {
      ...
      case Failure(e) if !isFatal(e) =>
        val retriesLeft = if (isTransient(e)) maxRetries else maxRetries map {
          _ - 1
        }
  ...
  def fatal(t: Throwable): Boolean = t match {
    case obs: ObsException if obs.getErrorCode.startsWith("Invalid") || obs.getErrorCode.startsWith("NoSuch") => true
